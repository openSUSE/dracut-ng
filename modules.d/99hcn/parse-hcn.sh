...
# HCN (Hybrid Cloud Network)
# Discovers HCN configurations from device-tree and sets up bonding
#
# Workflow:
# 1. Discover HCN devices from device-tree (/proc/device-tree)
#    - Scans PCI devices (SR-IOV ethernet adapters)
#    - Scans vdevices (VNIC and Virtual Ethernet)
# 2. Build MAPPINGS: bond names -> slave devices with MACs and modes
# 3. Process kernel command line (ip=, rd.route=) and replace slave references
#    with bonds
# 4. Generate NetworkManager connections with nm-initrd-generator
# 5. Fix up generated NetworkManager connections to use correct bond masters and
#    naming
# 6. Reload NetworkManager connections via nm_reload_connections()
#
# Usage:
# - Add rd.hcn=1 to kernel command line to explicitly enable HCN configuration
#   (it will be automatically enabled if the hardware populates
#   /proc/device-tree)
# - Use standard ip= and rd.route= parameters
...
for BONDNAME in $BOND_NAMES; do
    ...
    # Process ip= - replace slave names/MACs with bond name
    if [ -n "$HCN_IP" ]; then
        matched=0
    ...
    # Process rd.route= - replace slave names/MACs with bond name
    if [ -n "$HCN_ROUTE" ]; then
        matched=0
        for slave in $SLAVE_NAMES $SLAVE_MACS; do
            slave_dash=$(str_replace "$slave" ":" "-")
    ...
    for slave in $SLAVE_NAMES $SLAVE_MACS; do
        ...
        if strstr "$MOD_CMDLINE" "$slave" || strstr "$MOD_CMDLINE" "$slave_dash"; then
            info "parse-hcn: replacing slave $slave in cmdline with $BONDNAME"
...
if [ $CHANGED -eq 1 ] || [ -n "$NEW_ARGS" ]; then
    info "parse-hcn: writing /run/initramfs/cmdline.d/99-hcn.conf"
    if ! echo "$NEW_ARGS" > /run/initramfs/cmdline.d/99-hcn.conf; then
        warn "parse-hcn: failed to write configuration file"
        return 1
    fi

    CMDLINE="$MOD_CMDLINE $NEW_ARGS"
